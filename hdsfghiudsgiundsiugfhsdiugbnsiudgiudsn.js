// Steal the reference to the ui (I also cancelled any App.main calls in index.html, so only 1 instance :))


// Looping on all the "cells" in the graph.
for(let cellIndex in window.hello.editor.graph.model.cells){
    let cell = window.hello.editor.graph.model.cells[cellIndex]
	// Check if a "cell" has a text which equals to 'Start'
    if(cell.value === 'Start'){
        console.log(cell)
    }
}
...
let ahmed = new mxCell("Ahmed", new mxGeometry(0, 120, 120, 80), "ellipse;whiteSpace=wrap;html=1;")
...
// Sometimes a graph component update is required?
//window.hello.editor.updateGraphComponents()

function initiator() {
	let theGrid = document.querySelector(".geBackgroundPage");
	...
	for(let cellIndex in window.hello.editor.graph.model.cells){
		...
		if(((typeof cell.value === 'string' && cell.value === 'התחלה') || (typeof cell.value === 'object' && cell.value.getAttribute('label') === 'התחלה') && cell.value.getAttribute("makey") === 'start') && style.shape === "ellipse" && style.fillColor === '#d5e8d4'){
	...
	window.hello.editor.graph.addListener(mxEvent.REMOVE_CELLS, function(sender, evt) {
		...
		if(evt.properties.cells.length > 0) {
			...
			if(window.hello.editor.graph.getStylesheet().getCellStyle(evt.properties.cells[0].style).fillColor == '#ffe6cc'
			...
			&& typeof evt.properties.cells[0].value === 'object'
			&& evt.properties.cells[0].value.getAttribute('makey') === 'node_message') {
				window.hello.editor.graph.addCell(evt.properties.cells[0], window.hello.editor.graph.model.cells[evt.properties.cells[0].value.getAttribute('myparent')])
...
App.main(function(ui){window.hello = ui})
...
window.hello.editor.graph.addListener(null, function(sender, evt) {
	...
	if(CREATION_EVENT_FLOW[currentFlow.length] === evt.name) {
		currentFlow.push(evt.name);
		if(currentFlow.length === CREATION_EVENT_FLOW.length){
...
EditorUi.prototype.showDataDialog = function(a) {
    null != a && (a = new EditDataDialog(this,a),